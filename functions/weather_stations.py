...
def get_quality_stations(weather_station: pd.DataFrame) -> list:
    ...
    station_df = get_station_df(
        quality_station.loc["Number"], dt.now().year - 21, dt.now().year - 1
    )
...
def to_list_dfs(endYear: int, nearest_station: pd.DataFrame) -> list:
    ...
    for i in nearest_station.index:
        stations_dfs.append(
            get_station_df(nearest_station.loc[i, "Number"], startYear, endYear)
        )

    return stations_dfs


def weighted_ave_col(
    input_dfs, colname: str, nearest_station_df: pd.DataFrame, selected_stations: list
) -> pd.Series:
    if isinstance(input_dfs, list):
        weights = []
        for station in selected_stations:
            weights.append(
                nearest_station_df[nearest_station_df.iloc[:, 0] == station][
                    "weights"
                ].iloc[0]
            )
        out = 0
...
def annual_summary(df: pd.DataFrame, assess_year: int) -> tuple:
    ...
    eto_short = df["ETShortCrop"].loc[df["Year"].eq(assess_year)].mean()
    eto_tall = df["ETTallCrop"].loc[df["Year"].eq(assess_year)].mean()
    return rain, eto_short, eto_tall


def longTerms_summary(df: pd.DataFrame) -> tuple:
    rain = df["Rain"].sum() / 20
    eto_short = df["ETShortCrop"].mean() / 20
    eto_tall = df["ETTallCrop"].mean() / 20
    return rain, eto_short, eto_tall


def get_nearby_stations(
    lat: float, long: float, station_df: pd.DataFrame
) -> pd.DataFrame:
    ...
    for i in station_df_copy.index:
        row_coords = (
            station_df_copy.loc[i, "Latitude"],
            station_df_copy.loc[i, "Longitud"],
        )
    ...
    station_df_copy.insert(
        len(station_df_copy.columns), "distance to polygon", distances
    )
    ...
    select_station_df = pd.DataFrame(
        columns=station_df_copy.columns.to_list() + ["Fraction of ranfall from BOM"]
    )
    i = 0
    while len(select_station_df) < 8 and i < len(station_df_copy):
        ...
        if quality_station is not None:
            select_station_df = pd.concat(
                [select_station_df, quality_station.to_frame().T], ignore_index=True
            )
        i += 1
    ...
    return select_station_df.drop(
        ["Latitude", "Longitud", "Stat", "Elevat.", "Distance (km)"], axis=1
    )
