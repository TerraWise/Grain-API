...
def _safe_get(df, col, fallback, split_on=None):
    try:
        val = df[col].iloc[0]
        return val.split(split_on) if split_on else val
    except (KeyError, AttributeError):
        return fallback


def _load_crop_csv(path, cols_to_drop):
    return pd.read_csv(path).drop(cols_to_drop, axis=1)


def _extract_product_name(df, col, which, crop_lower, row_idx):
    parts = df[col].iloc[row_idx].split("_")
    if "".join(parts) == "other":
        parts = df[f"specify_{which}_{crop_lower}"].iloc[row_idx].split("_")
    return " ".join(parts).capitalize()


def _get_area(df, which, crop, row_idx, whole_area):
    col_variants = [
        f"{which}_hectares_spec_{crop}",
        f"{which}_specify_hectares_{crop}",
        f"{which}_hectares_{crop}_spec",
    ]
    for variant in col_variants:
        try:
            return float(df[variant].iloc[row_idx])
        except KeyError:
            continue
    return float(whole_area)


def from_the_top(zipfiles: list):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            for csv in os.listdir(td):
                if "questionnaire" in csv.lower():
                    questionnaire_df = pd.read_csv(os.path.join(td, csv))
            crops = questionnaire_df["crops_grown"].iloc[0].split(",")
            crop_specific_input = crop_assemble(td, crops)
            veg_df = vegetation_df(td)
    return crops, crop_specific_input, questionnaire_df, veg_df


def crop_assemble(tmp_input_dir: str, crops: list) -> dict:
    cols_to_drop = [
        "ObjectID",
        "GlobalID",
        "ParentGlobalID",
    ...
    for crop in crops:
        ...
        for filename in os.listdir(tmp_input_dir):
            ...
            if parts[0] not in ("chem", "machine"):
                canola_in_csv = "_".join(parts[1:3])
            else:
                canola_in_csv = "_".join(parts[2:4])
            if canola_in_csv == crop or ("gm_canola" not in crop and crop in filename):
                key = "_".join(parts[:2]) if "chem" in parts else parts[0]
                crop_specific_input[crop][key] = _load_crop_csv(
                    os.path.join(tmp_input_dir, filename), cols_to_drop
                )
    return crop_specific_input


def vegetation_df(tmp_input_dir: str) -> pd.DataFrame:
    for csv in os.listdir(tmp_input_dir):
        if "planting" in csv:
            return pd.read_csv(os.path.join(tmp_input_dir, csv))
    return None


# Extract follow up question
def follow_up(df: pd.DataFrame, dir: str):
    # Extract from csv
    info = {}
    info["List of on-farm machinery"] = _safe_get(
        df,
        "If you have a list of all on-farm machinery and equipment, please upload it here. Alternatively, please email it to toby@terrawise.au",
        "Don't have attachment, please follow up",
        split_on="\n",
    )
    ...
    info["Access to software & \n record of variable rate"] = _safe_get(
        df,
        "Are you happy to provide us with access to these applications, record and/or service providers to conduct your carbon account?",
    ...
    if (
        df["Do you engage any on-farm contractor services during the year?"].iloc[0]
        == "yes"
    ):
        try:
            info["Contractor activities"] = df["Select all that apply"][0].split(",")
        except AttributeError:
            info["Contractor activities"] = (
                "Yes engagement with contractor but didn't select the activities"
            )
    else:
        info["Contractor activities"] = (
            "No contractor activites on-farm during preivoues production year"
        )
...
def land_management(df: pd.DataFrame, crops: list, dir: str):
    ...
    for crop in crops:
        ...
        try:
            ...
            crop_info[f"Land management practices - {crop}"] = (
                "Wasn't answered in the form"
            )
...
def list_fert_chem(
    input_dict: dict, crops: list, questionnaire_df: pd.DataFrame, which: str
) -> dict:
    products_applied = {}
    name_keyword = "npk" if which == "fert" else "applied"
    for crop in crops:
        ...
        rates = []
        forms = []
        whole_area = questionnaire_df[f"area_sown_{crop_lower}"].iloc[0]
        area = []
        times = []
        for col in df.columns:
            for i in df.index:
                col_lower = col.lower()
                if name_keyword in col_lower:
                    names.append(_extract_product_name(df, col, which, crop_lower, i))
                ...
                if "hectares" in col_lower:
                    if df[col].iloc[i] == "whole":
                        ...
                        area.append(_get_area(df, which, crop, i, whole_area))
...
# Soil amelioration
def to_soil_ame(df: pd.DataFrame, crops: list) -> dict:
...
# Vegetation
def to_veg(veg_df, planting_shapes) -> dict:
...
def get_planting_region(shapes):
    BOM_rainfall_gdf = gpd.read_file(os.path.join("input", "BOM_RF_Region.zip"))
